...
/// @notice Interface for spend permission hook contracts
interface ISpendHook {
    /// @notice Apply hook logic before token transfer
    /// @param spendPermission Details of the spend permission
    /// @param value Amount being spent
    /// @param hookData Arbitrary data for the hook to process
...
contract Permit3 is EIP712 {
    ...
    /// @notice Hook delegate call failed
    error HookDelegateCallFailed();
    ...
    /// @dev Executes hook logic via delegateCall if a delegate is registered for this permission.
    ///
    /// @param spendPermission Details of the spend permission.
    /// @param value Amount of token attempting to spend.
    /// @param hookData Arbitrary data to pass to the hook delegate.
    function spend(SpendPermission memory spendPermission, uint160 value, bytes calldata hookData)
        external
        requireSender(spendPermission.spender)
    {
        ...
        if (hookDelegate != address(0)) {
            ...
            // We should probably at least log it though.
            if (!success) revert HookDelegateCallFailed();
        }
    ...
    function _transferFrom(address token, address account, address recipient, uint256 value) internal {
        if (token == NATIVE_TOKEN) {
            ...
            // Assume existing allowance for Permit3 was set up during approval of the permit
