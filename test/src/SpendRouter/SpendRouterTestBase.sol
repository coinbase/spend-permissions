...
contract SpendRouterTestBase is Test {
    ...
    function setUp() public virtual {
        ...
        permissionManager = new SpendPermissionManager(validator, address(magicSpend));
    ...
    function _createWithdrawRequest(SpendPermissionManager.SpendPermission memory permission, uint128 nonceEntropy)
        internal
        view
    ...
    function _signWithdrawRequest(address withdrawAccount, MagicSpend.WithdrawRequest memory withdrawRequest)
        internal
        view
        returns (bytes memory)
    {
        bytes32 hash = magicSpend.getHash(withdrawAccount, withdrawRequest);
