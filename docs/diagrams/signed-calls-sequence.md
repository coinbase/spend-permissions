...
sequenceDiagram
    ...
    A-->>H: success
    H->>H: set permit3 hash as signed
    ...
    A->>H: isValidSignature(hash, empty)
    Note over H: hash was previously marked as signed by acct
    H-->>A: return isSigned[msg.sender][hash]
    A-->>V: return success
    V-->>P: return success
